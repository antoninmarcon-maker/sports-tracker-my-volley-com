...
import { Plus, History, Trash2, Eye, Play, Info, CheckCircle2, LogOut } from 'lucide-react';
      ...
      <header className="px-4 py-6 border-b border-border flex flex-col items-center gap-3 relative">
        ...
        <div className="absolute top-4 right-4">
          {user ? (
            ...
            >
              <LogOut size={14} />
              {user.user_metadata?.full_name?.split(' ')[0] || 'Déconnexion'}
            </button>
          ) : (
            <button
              onClick={handleGoogleSignIn}
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-card border border-border text-foreground text-xs font-medium hover:bg-secondary transition-all"
            >
